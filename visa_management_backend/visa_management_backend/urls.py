...
from visa_management_app.views import StudentInfoViewSet,Reservable_datesViewSet,ReservationViewSet,UserProfileViewSet,GeneratePDFView,RequestVisaExtensionViewSet,SendTokenPaid90DaysEmail,SendEmailExtensionNotification,LogoutStudent,PayToken,SpecificTransactionList
...
urlpatterns = [
    ...
    # path('transactions/', TransactionView.as_view({'get': 'list'}), name='transaction-list'),
    path('transactions/<str:account_number>/', SpecificTransactionList.as_view(), name='user-transactions'),
    # path('api/user/<str:account_number>/paytoken', pay_token, name='user-paytoken'),
    path('api/user/<str:account_number>/paytoken/', PayToken.as_view(), name='user-paytoken'),
